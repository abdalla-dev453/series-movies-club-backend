...
def test_user_can_join_club(client):
    ...
    resp = client.post(f"/api/clubs/{club_id}/join", headers=headers_b)
...
def test_duplicate_join_is_rejected(client):
    ...
    resp = client.post(f"/api/clubs/{club_id}/join", headers=headers_a)
...
def test_last_admin_cannot_be_demoted(client):
    ...
    resp = client.put(
        f"/api/clubs/{club_id}/members/{user_a['id']}",
        headers=headers_a,
        json={"role": "member"},
    )
...
def test_last_admin_cannot_leave(client):
    ...
    resp = client.delete(f"/api/clubs/{club_id}/leave", headers=headers_a)
    assert resp.status_code == 400
