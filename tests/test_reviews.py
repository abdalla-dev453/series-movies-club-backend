...
def create_post(client, headers, title="The Matrix"):
    resp = client.post("/api/posts", headers=headers, json={"movie_title": title})
...
def test_cannot_review_own_post(client):
    ...
    resp = client.post("/api/reviews", headers=headers, json={"post_id": post_id, "rating": 5})
...
def test_duplicate_review_is_rejected(client):
    ...
    client.post("/api/reviews", headers=headers_b, json={"post_id": post_id, "rating": 4})
    resp = client.post("/api/reviews", headers=headers_b, json={"post_id": post_id, "rating": 5})
...
def test_invalid_rating_is_rejected(client):
    ...
    _, headers_b = signup_and_login(client, username="bob", email="b@example.com")
    resp = client.post("/api/reviews", headers=headers_b, json={"post_id": post_id, "rating": 9})
...
def test_author_can_edit_own_review(client):
    ...
    resp = client.put(f"/api/reviews/{review_id}", headers=headers_b, json={"rating": 5})
