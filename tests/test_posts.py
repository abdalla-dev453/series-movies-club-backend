...
def create_club(client, headers, name="Comedy Fans", genre="Comedy"):
    resp = client.post("/api/clubs", headers=headers, json={"name": name, "genre": genre})
    return resp.get_json()["id"]


def test_create_personal_post(client):
    _, headers = signup_and_login(client)
    resp = client.post("/api/posts", headers=headers, json={"movie_title": "Inside Out"})
...
def test_non_member_cannot_post_into_club(client):
    ...
    resp = client.post(
        "/api/posts", headers=headers_b, json={"movie_title": "Up", "club_id": club_id}
    )
...
def test_club_admin_can_delete_others_post(client):
    ...
    post_resp = client.post(
        "/api/posts", headers=headers_b, json={"movie_title": "Coco", "club_id": club_id}
    )
    post_id = post_resp.get_json()["id"]

    resp = client.delete(f"/api/posts/{post_id}", headers=headers_a)
