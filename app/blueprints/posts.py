...
@jwt_required()
def create():
    """Create a new personal or club post."""
    ...
    # If this is a club post, user must belong to that club.
    if club_id is not None:
        if not is_club_member(current_user.id, club_id):
            raise APIError(
                "You must be a member of this club to post there",
                403,
            )
...
def get_post_reviews(post_id):
    """Get reviews for one post."""
    get_or_404(Post, post_id)
    reviews = Review.query.filter_by(post_id=post_id).order_by(Review.created_at.asc()).all()
...
def delete(post_id):
    ...
    A club admin can delete a post shared in their club.
    """
    current_user = get_current_user()
    post = get_or_404(Post, post_id)
    ...
    is_admin_of_its_club = (
        post.club_id is not None
        and is_club_admin(
            current_user.id,
            post.club_id,
        )
